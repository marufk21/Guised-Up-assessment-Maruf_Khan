...
Use `maruf@example.com` as the demo user. For iOS Simulator use `http://127.0.0.1:8000/api`; for Android Emulator use `http://10.0.2.2:8000/api`; for a physical device use a reachable LAN URL and start Laravel on a reachable interface.
...
Introduce yourself as Maruf Khan and describe the Real Connections Feed as a focused social-feed proof of concept. The goal is to rank authentic, personally relevant posts without using platform-wide popularity metrics such as total likes or views.
